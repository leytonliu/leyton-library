  ...
  <view :id="tabId" :class="classes" :data-component="data.componentCode" :style="styles" @tap="handleTapBaseContainer">
    ...
    <template v-for="(tab, tabIndex) in tabs">
      <view v-if="!!tab.init" :key="tab.title" :style="tabStyles[tabIndex]" class="cms-tabs-content">
        <cms-base-component v-for="(item, index) in tabsProductChildrenData" :key="item.componentId" :children-styles="tabsProductChildrenStyles[index]" :data="item" :index="index" />
      </view>
    </template>
...
<script>
  ...
  export default {
    ...
    /*
     * @Description: 兼容H5: 监听时机需要前置在beforeCreate中，确保一定能拿到hasFixedContainer的数据
     * @Author: sunliu
     * @Date: 2022-11-01 14:39:25
     */
    beforeCreate() {
      uni.$on('hasFixedContainer', ({ height }) => {
        ...
        this.headerStickyTop = height + uni.upx2px(this.env.navHeight * 2) + this.env.fixedTopHeight + this.statusBarHeight;
      });
    },
    ...
    methods: {
      adjustStyles(styles) {
        if (this.isSticky) {
          styles.minHeight = `calc(100vh - env(safe-area-inset-bottom) - env(safe-area-inset-top) - var(--TABBAR-HEIGHT) - ${this.headerStickyTopStyles} + ${styles.paddingTop || 0} + ${styles.paddingBottom || 0})`;
        } else {
      ...
      async getProductListByCode(tab) {
        const { data, count } = await this.env.getTabProducts({
          code: tab.groupCode,
          size,
        ...
        this.tabs.splice(tab.index, 1, {
          ...tab,
          list: tab.list.concat(data),
          page: tab.page + 1,
          ...((tab.page + 1) * size <= count && { maxScore: (tab.page + 1) * size }),
        });
