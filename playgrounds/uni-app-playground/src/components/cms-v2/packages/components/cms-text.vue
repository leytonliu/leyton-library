<template>
  <view v-if="!!getBindingValue(data.data.text) && textEventStop" :class="classes" :data-component="data.componentCode" :style="styles" @tap.stop="handleTapBaseContainer">
    <view :style="textStyles">
      {{ getBindingValue(data.data.text) }}
    </view>
  </view>
  <view v-else-if="!!getBindingValue(data.data.text)" :class="classes" :data-component="data.componentCode" :style="styles" @tap="handleTapBaseContainer">
    <view :style="textStyles" :class="data.data.fontFamily || 'font-regular'">
      <view v-if="data.data.text === '@@product-label'" class="label-wrap">
        <view class="label mr4" v-for="(label, index) in getBindingValue(data.data.text).split(' ')" :key="index">{{ label }}</view>
      </view>
